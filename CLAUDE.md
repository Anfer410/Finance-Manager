...
1. **At the start of each conversation**, create a branch from `main`:
   ...
   ```
2. **Open a draft MR immediately** targeting `develop` so the work is visible early.
3. **Commit when each logical unit of work is done** — don't batch unrelated changes. Use conventional commits (`feat:`, `fix:`, `chore:`, etc.).
