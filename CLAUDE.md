...
**Every conversation = a new branch.** Never work directly on `main`.

1. **At the start of each conversation**, create a branch named after the work (e.g. `feat/add-export-button`, `fix/loan-calc-rounding`):
   ```bash
...
When an MR is open, GitLab CI automatically builds a pre-release Docker image tagged `mr-<MR_IID>` and pushes it to the GitLab container registry:

```
registry.gitlab.iveydomek.xyz/scripts/finances/finance-manager:mr-42
```

To test in the home environment, update `docker-compose.yml` to use that tag, then `docker compose pull && docker compose up -d`.

## Release Notes

- **`CHANGELOG.md`** — auto-generated by `python-semantic-release` on every merge to `main`; committed to the repo
- **GitLab Releases** — created automatically by `semantic-release publish` at the same time; visible under the project's Releases page with the same changelog content
- Human-curated notes belong in the MR description before merging
