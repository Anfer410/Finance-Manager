...
def _family_filter() -> tuple[str, dict]:
    """Returns (sql_clause, params_dict) for filtering by the current family."""
...
def get_employer_income_series(year: int, persons: list[int] | None = None) -> dict:
    ...
    if cfg.employer_pattern_strings:
        employer_clause = "(" + " OR ".join(
            f"description ILIKE '%{p}%'" for p in cfg.employer_pattern_strings
        ) + ")"
    ...
    return {
        ...
        "has_employer_patterns": bool(cfg.employer_pattern_strings),
    }
