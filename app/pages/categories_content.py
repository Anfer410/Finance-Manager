...
def content() -> None:
    ...
    def _save_fn() -> None:
        save_category_config(cfg, auth.current_family_id())
        app.storage.user['views_dirty'] = True
    ...
    with ui.row().classes('w-full gap-4 items-start flex-wrap mt-4'):
            ...
            with ui.row().classes('items-center justify-between mb-3'):
                ...
                with ui.row().classes('gap-1'):
                    ui.button(icon='delete_sweep',
                              on_click=lambda: _clear_rules_dialog(cfg, rule_table, _save_fn)) \
                        .props('flat round dense').classes('text-red-400') \
                        .tooltip('Clear all rules')
                    ui.button(icon='add', on_click=lambda: _add_rule_dialog(cfg, rule_table, _save_fn)) \
            ...
            def rule_table() -> None:
                for rule in cfg.sorted_rules():
                    with ui.row().classes('items-center gap-2 py-1 border-b border-gray-50 w-full'):
                        ...
                        ui.button(icon='edit',
                                  on_click=lambda _, r=rule: _edit_rule_dialog(r, cfg, rule_table, _save_fn)) \
                            .props('flat round dense size=xs').classes('text-gray-400')
                        ui.button(icon='delete',
                                  on_click=lambda _, r=rule: _delete_rule(r, cfg, rule_table, _save_fn)) \
        ...
        with ui.element('div').classes('card flex-1').style('min-width:280px;max-width:380px'):
            with ui.row().classes('items-center justify-between mb-3'):
                ...
                ui.button(icon='add', on_click=lambda: _add_category_dialog(cfg, category_table, _save_fn)) \
                    .props('flat round dense').classes('text-gray-500')
...
def _run_suggestions(state: dict, body_fn) -> None:
    state["clusters"] = get_uncategorized_clusters(auth.current_family_id())
    body_fn.refresh()
        ...
        with ui.row().classes('justify-end gap-2 w-full'):
            ...
            def _ok():
                ...
                if pat and cat:
                    ...
                    save_fn()
                    _run_suggestions(suggestions_state, suggestions_body_fn)
...
def _add_category_dialog(cfg: CategoryConfig, refresh_fn, save_fn) -> None:
...
def _delete_category(cat: Category, cfg: CategoryConfig, cat_fn, rule_fn, save_fn) -> None:
...
def _add_rule_dialog(cfg: CategoryConfig, refresh_fn, save_fn) -> None:
    with ui.dialog() as dlg, ui.card().classes('w-96 gap-3'):
...
def _edit_rule_dialog(rule: CategoryRule, cfg: CategoryConfig, refresh_fn, save_fn) -> None:
    with ui.dialog() as dlg, ui.card().classes('w-96 gap-3'):
...
def _delete_rule(rule: CategoryRule, cfg: CategoryConfig, refresh_fn, save_fn) -> None:
    cfg.rules = [r for r in cfg.rules if r is not rule]
    refresh_fn.refresh()
    save_fn()


def _clear_rules_dialog(cfg: CategoryConfig, refresh_fn, save_fn) -> None:
    with ui.dialog() as dlg, ui.card().classes('w-80 gap-3'):
